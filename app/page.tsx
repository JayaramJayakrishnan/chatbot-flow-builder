...
import CustomSnackbar, { showAlert } from "./_components/Snackbar";
...
export default function ChatbotFlowBuilder() {
    ...
    (params: Connection) => {
      ...
      if (sourceHasEdge) {
        setShowAlert({
          show: true,
          message:
            "A node can only have one outgoing edge from its source handle!",
          status: "error",
        });
        return;
      }
  ...
  const handleSaveFlow = useCallback(() => {
    ...
    if (nodes.length === 0) {
      ...
      setShowAlert({
        show: true,
        message: "Please add at least one node to the flow!",
        status: "warning",
      });
      return;
    }
